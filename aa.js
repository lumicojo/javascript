...
function showDetails(char) {
  ...
  document.querySelector('#vote-count').textContent = char.votes
...
function renderCharacter(char) {
  ...
  const info = document.querySelector('#character-bar')
  ...
  function handleAddVotes(e) {
    e.preventDefault();
    const numVotes=document.querySelector('#votes').value
    let votesSpan =document.querySelector('#vote-count')
    let currentVotes = votesSpan.textContent
    console.log(numVotes + 1)

    votesSpan.textContent = parseInt(numVotes) + parseInt(currentVotes)
    e.target.reset()
...
fetch('http://localhost:3000/characters')
   ...
   .then(data => {
    ...
    document.querySelector('#votes-form')
    .addEventListener('submit',handleAddVotes)
   })
